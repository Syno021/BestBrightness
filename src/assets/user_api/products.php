...
elseif ($_SERVER['REQUEST_METHOD'] === 'PUT') {
    ...
    if (isset($postdata) && isset($query['id']) && !empty($query['id'])) {
        ...
        // Ensure total_ratings and average_rating are set
        $total_ratings = isset($request->total_ratings) ? (int)$request->total_ratings : 0;
        $average_rating = isset($request->average_rating) ? (float)$request->average_rating : 0.00;
